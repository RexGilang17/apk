  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: () => Get.toNamed(Routes.addCategoryProduct),
                icon: Icon(Icons.add))
          ],
        ),
        body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data!.docs.isEmpty) {
              return const Center(
                child: Text("Jenis Barang masih kosong",
                    style:
                          ...
                          IconButton(
                              onPressed: () {
                                Get.defaultDialog(
                                    ...
                                    middleText:
                                        "Apakah kamu yakin untuk hapus jenis barang ini?",
                                    actions: [
                                      OutlinedButton(
                                          onPressed: () => Get.back(),
                                            ...
                                            if (!res['error']) {
                                              ScaffoldMessenger.of(context)
                                                  .showSnackBar(const SnackBar(
                                                      content: Text(
                                                          "Berhasil Hapus Jenis Barang")));
                                            } else {
                                              ScaffoldMessenger.of(context)
                                                  .showSnackBar(const SnackBar(
                                                      content: Text(
                                                          "Gagal Hapus Jenis Barang")));
                                            }
                                          },
